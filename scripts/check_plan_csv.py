...
from id8_common.plans.acquire.plan_csv import read_plan_csv  # noqa: E402


def read_yaml(file_path):
    """Local mirror of master_plan.read_yaml's suffix dispatch.

    Not imported from master_plan, which is the real dispatch point, because that
    module does `pv_registers = oregistry["pv_registers"]` at import time and so
    needs a populated registry -- i.e. a live session. This script has to run on
    kouga with no EPICS at all. Keep these four lines identical to the ones in
    master_plan.read_yaml; they are the only thing duplicated, and the real
    dispatch is exercised the first time a session is pointed at a .csv.
    """
...
def main(argv):
    if len(argv) >= 2:
        ...
        pairs = [
            ...
            # One detector -> the serial shape master_plan.py reads.
            (
                EXAMPLES / "measurement_info_serial.csv",
                EXAMPLES / "measurement_info_serial.expected.yaml",
            ),
            # Three detectors -> the detectors: list dual_master_plan.py reads.
            (
                EXAMPLES / "measurement_info_trio.csv",
                EXAMPLES / "measurement_info_trio.expected.yaml",
            ),
        ]
