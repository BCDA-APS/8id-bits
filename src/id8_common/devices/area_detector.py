...
def ad_setup(det: AreaDetector, iconfig: dict) -> None:
    ...
    # No HDF plugin priming here, deliberately -- everything above is in-memory,
    # and ad_setup() must not touch hardware. apstools' AD_prime_plugin2() fires a
    # REAL exposure, and AD_plugin_primed() gates it on cam.data_type ==
    # hdf1.data_type, which is permanently false on the eiger4M (cam Int8 vs HDF1
    # UInt32, re-verified against live PVs 2026-09-14). So every single session
    # start fired an exposure -- which crashed measurement G0209 on 2026-09-12
    # when a second session was started mid-acquisition.
    #
    # Priming is not needed by any of them: eiger4M, lambda2M and rigaku3M all run
    # hdf1 LazyOpen=Yes in Stream mode (verified against live PVs 2026-09-14),
    # which per apstools' own AD_plugin_primed docstring removes the need to prime.
    #
    # `iconfig` is now unused here but kept in the signature for the call sites in
    # startup.py. ALLOW_AREA_DETECTOR_WARMUP in iconfig.yml is correspondingly
    # dead, and is set False so nothing reads it as still live.
