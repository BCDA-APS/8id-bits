...
This module assembles those into ACQ_MODES and provides the serial acquisition
...
Its counterpart is multi_acq.multi_acq_series(), which runs several detectors in
ONE beam window. It reuses the setup half of ACQ_MODES verbatim and replaces
only the acquire half, which cannot be run twice at once because it owns the
shutter; it lived here from 2026-09-11 to 2026-09-14. Both loops are driven from
master_plan.py, off the SAME measurement_info.yaml and sample_info.yaml: a
protocol with a scalar ``detector:`` takes this one, one with a ``detectors:``
list takes multi_acq.
