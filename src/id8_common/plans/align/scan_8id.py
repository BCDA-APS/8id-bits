...
def dmesh(
    ...
    usage:
        RE(dmesh(huber.x, -0.5, 0.5, 11, huber.z, -0.5, 0.5, 11, 1))
    ...
    args:
        ...
        count_time: detector acquisition time per point (s)
        det: detector (lambda2M, eiger4M, or tetramm1)
        att_ratio: attenuation ratio
    ...
    if is_lambda:
        ...
        def step_lambda(step_dict, pc, frame_num):
            yield from bps.move_per_step(step_dict, pc)
            softglue.start_pulses.put("1!")
            # wait for this frame to be captured before reading stats / next trigger
            while det.hdf1.num_captured.get() < frame_num:
                yield from bps.sleep(0.005)
            yield from bps.create("primary")
        ...
        def inner_lambda():
            ...
            try:
                for p1 in positions1:
                    for p2 in positions2:
                        frame_num += 1
                        yield from step_lambda({motor1: p1, motor2: p2}, pos_cache, frame_num)
            finally:
                softglue.stop_pulses.put("1!")
                det.cam.acquire.put(0)
                det.hdf1.capture.put(0)
...
def mesh(
    ...
    motor1 is the outer (slow) axis, motor2 is the inner (fast) axis. For every
    position of motor1 the plan steps motor2 across its full range, counting the
    detector at each of the num1 * num2 grid points.

    usage:
        RE(mesh(huber.x, -0.5, 0.5, 11, huber.z, -0.5, 0.5, 11, 1))
