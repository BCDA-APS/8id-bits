...
from legacy.id8_e.plans.sample_info_unpack import select_sample
# from id8_e.plans.select_detector import select_detector
from legacy.id8_e.plans.scan_8ide import att
from legacy.id8_e.plans.nexus_acq_eiger_ext import eiger_acq_ext_trig
from legacy.id8_e.plans.nexus_acq_eiger_int import eiger_acq_int_series
        ...
        for block_key, block_value in measurement_info.items():
            ...
            for ii in range(len(att_list)):
                ...
                for jj in range(len(acq_time_list[ii])):
                    ...
                    if det_name == "lambda2M":
                        if acq_time == acq_period:
                            print(f"Using lambda2M internal series")
                            lambda_acq_int_series(
                                acq_time=acq_time,
                                num_frames=num_frames,
                                num_reps=num_reps,
                                wait_time=wait_time,
...
def set_temp_lakeshore2(temp, wait):
