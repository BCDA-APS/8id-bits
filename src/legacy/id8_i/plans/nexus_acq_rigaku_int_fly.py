...
def rigaku_ZDT_fly_acquire(fly_speed):
    """Run the Rigaku ZDT acquisition sequence."""

    acq_time = rigaku3M.cam.acquire_time.get()
    num_frame = rigaku3M.cam.num_images.get()

    # There's a significant delay from the time acq button is pressed 
    # to when the acquisition acutally starts
    extra_acq_time = 3.0 
    sample_y_orig = sample.y.user_setpoint.get()

    total_travel = fly_speed * (acq_time * num_frame + extra_acq_time)
    ...
    yield from bps.sleep(extra_acq_time)
...
def rigaku_acq_ZDT_fly_series(
    ...
    for ii in range(num_reps):
        ...
        yield from rigaku_ZDT_fly_acquire(fly_speed)
