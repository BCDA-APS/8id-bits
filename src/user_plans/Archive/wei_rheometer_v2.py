...
repeat_1ms_list[6] = 8
repeat_1ms_list[7] = 9
repeat_1ms_list[8] = 10
repeat_1ms_list[11] = 8
repeat_1ms_list[12] = 8
repeat_1ms_list[13] = 9
repeat_1ms_list[14] = 9
repeat_1ms_list[15] = 10
repeat_1ms_list[16] = 10
repeat_1ms_list[17] = 11
repeat_1ms_list[18] = 11
repeat_1ms_list[24] = 8
repeat_1ms_list[26] = 9
...
def rheo_xpcs_measure():
    ...
    for interval_idx, freq in enumerate(freq_list):
        ...
        rheometer.y.move(current_pos)
        pos_idx += 1

        wait_for_mcr()
        frame_rate = frame_rate_list[interval_idx]
        frame_num  = round(frame_num_list[interval_idx])
        repeat_1m  = round(repeat_1ms_list[interval_idx]-1)


        for rep_idcx in np.arange(repeat_1m):
        ##UNCOMMENT AND RE-ADD SHEAR RATE FOR SHEAR TESTING!!!##

            suffix = f"Int{(interval_idx+1):02d}_freq{round(freq*1e3):04d}_rep{round(rep_idcx):02d}_shear{round(shear_rate*1e1):02d}"
