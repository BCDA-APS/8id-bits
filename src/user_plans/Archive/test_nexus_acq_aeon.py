...
from legacy.id8_i.utils.dm_util import dm_run_job
from legacy.id8_i.utils.dm_util import dm_setup
from legacy.id8_i.utils.nexus_utils import create_nexus_format_metadata

from legacy.id8_i.plans.sample_info_unpack import gen_folder_prefix
from legacy.id8_i.plans.sample_info_unpack import mesh_grid_move
from legacy.id8_i.plans.shutter_logic import blockbeam
from legacy.id8_i.plans.shutter_logic import post_align
from legacy.id8_i.plans.shutter_logic import showbeam
from legacy.id8_i.plans.shutter_logic import shutteroff
...
def aeon_acquire(file_header, file_name):
    ...
    remote_cmd = f"LD_PRELOAD=libvma.so /home/xspadmin/tpx_env/daq_aps/acquire_event_based.py -s 1 -ds 1000 -th1 7476 -th3 7585 -fp /mnt/ramdisk/{file_name}"
    ...
    subprocess.run(["mkdir", "-p", f"{file_path}"], check=True)
